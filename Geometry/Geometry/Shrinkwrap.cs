    ...
    public static class Shrinkwrap
    {
        ...
        public static Mesh Wrap(Mesh source, Mesh target, ShrinkwrapMode mode, VertexProjection.ProjectionAxis axis = VertexProjection.ProjectionAxis.None, ProjectionMissResponse onMiss = ProjectionMissResponse.None)
        {
            ...
            if(mode == ShrinkwrapMode.Project)
            {
                ...
                {
                    Mesh delaunaryMesh = DelaunayTriangulation.Triangulate(targetCopy.Vertices, new Func<Vertex, DelaunayPoint>(v =>
                    {
                    ...
                    foreach (Vertex v in delaunaryMesh.Vertices)
                    {
                        v.UV = getUV(v.Position);
                    }
                    delaunaryMesh.HasUVs = true;
                    delMo = new MeshOperator(delaunaryMesh, false, false, true);
                }
