...
# Runs process-contextual --master as a service on an EC2 instance.
#
# See contextual-master.sh for localhost use.
...
eval "$landform configure --venue=contextual-master-service $quiet"

while true; do
  eval "$landform process-contextual --stacktraces --master $quiet"
  sleep 30
done
