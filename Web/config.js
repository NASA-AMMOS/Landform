...
const dbg = process.argv.slice(2).some(a => {
  ...
  return la === '-d' || la === '--debug' || la === '--debug=true';
});
...
const development = {
  app: {
    ...
    deployEnvironment: env.DEPLOY_ENV || 'landformweb-dev',
    ...
    binDir: env.BIN_DIR || path.join(cwd, '..', 'TilingServer', 'bin', dbg ? 'Debug' : 'Release'),
    ...
    venueName: env.TILE_SERVER_VENUE_NAME || 'landformweb-dev',
...
production.app.deployEnvironment = env.DEPLOY_ENV || 'landformweb';
production.app.sessionCookieSecure = !env.WITHOUT_HTTPS;
production.app.logLevel = env.LOG_LEVEL || 'verbose';
production.app.binDir = env.BIN_DIR || path.join(cwd, 'bin');
