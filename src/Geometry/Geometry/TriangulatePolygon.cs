...
namespace JPLOPS.Geometry
{
    public static class TriangulatePolygon
    {
        ...
        public static List<Edge> Triangulate(List<Edge> perimeter, bool ccw = true)
        {
            ...
            //modeled after vertexTree in MeshOperator
            RTree<int> vertexTree = new RTree<int>(MaxNodeEntries: 10, MinNodeEntries: 5);
            for (int i = 0; i < perimeter.Count; ++i) {
                Vector3 v = perimeter[i].Src.Position;
                v.Z = 0;
                vertexTree.Add(v.ToRectangle(), i);
            }
                ...
                for (int i = 0; i < perimeter.Count - 1; ++i) //Simple polygon should have at least 2 ears
                {
                    ...
                    if (ccw == Edge.IsLeftTurn(e1, e2, 1E-8))
                    {
                        ...
                        if (tri.Area() > 1E-4) //Allow degenerate triangles to handle colinear points
                        {
                            ...
                            foreach (int j in NearestVertices(perimeter, vertexTree, center, testDistSq + 1E-8))
                            {
                                Vertex v = perimeter[j].Src;
        ...
        public static List<int> NearestVertices(List<Edge> perimeter, RTree<int> vertexTree, Vector3 center, 
                                                double nearestDistSq)
        {
            double nearestDist = Math.Sqrt(nearestDistSq);
            var min = center - new Vector3(nearestDist);
            var max = center + new Vector3(nearestDist);
            ...
            {
                if (Vector3.DistanceSquared(center, perimeter[i].Src.Position) <= nearestDistSq)
                {
                    result.Add(i);
                }
            }
