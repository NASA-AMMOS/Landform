    ...
    public class ComputeHelper : IDisposable
    {
        ...
        public static string GetSelfInstanceID(ILogger logger = null)
        {
            ...
            try
            {
                ...
                {
                    var response = client.GetAsync("http://169.254.169.254/latest/meta-data/instance-id").GetAwaiter().GetResult();
                    if (!response.IsSuccessStatusCode)
                    {
                        throw new Exception($"HTTP {response.StatusCode} {response.ReasonPhrase}");
                    }
                    id = response.Content.ReadAsStringAsync().GetAwaiter().GetResult();
                    if (logger != null)
                    {
        ...
        public List<string> InstanceNamePatternToIDs(string namePattern, InstanceState state = InstanceState.unknown)
        {
            ...
            try
            {
                ...
                do
                {
                    var resp = ec2Client.DescribeInstancesAsync(req).GetAwaiter().GetResult();
        ...
        public InstanceState GetInstanceState(string instanceID)
        {
            ...
            try
            {
                ...
                var resp = ec2Client.DescribeInstanceStatusAsync(req).GetAwaiter().GetResult();
                if (resp.HttpStatusCode != HttpStatusCode.OK)
                {
                    throw new Exception($"HTTP {resp.HttpStatusCode}");
                }
        ...
        public bool StartInstances(params string[] instanceIDs)
        {
            try
            {
                ...
                if (ids.Count > 0)
                {
                    ...
                    var resp = ec2Client.StartInstancesAsync(req).GetAwaiter().GetResult();
                    if (resp.HttpStatusCode != HttpStatusCode.OK)
                    {
                        throw new Exception($"HTTP {resp.HttpStatusCode}");
                    }
        ...
        public bool StopInstances(params string[] instanceIDs)
        {
            try
            {
                ...
                if (ids.Count > 0)
                {
                    ...
                    var resp = ec2Client.StopInstancesAsync(req).GetAwaiter().GetResult();
                    if (resp.HttpStatusCode != HttpStatusCode.OK)
                    {
                        throw new Exception($"HTTP {resp.HttpStatusCode}");
                    }
        ...
        public bool SetAutoScalingGroupSize(string name, int min, int desired, int max)
        {
            ...
            try
            {
                ...
                var resp = asClient.UpdateAutoScalingGroupAsync(req).GetAwaiter().GetResult();
                if (resp.HttpStatusCode != HttpStatusCode.OK)
                {
                    throw new Exception($"HTTP {resp.HttpStatusCode}");
                }
        ...
        public bool SetInstanceProtection(string asgName, string instanceID, bool enabled)
        {
            ...
            try
            {
                ...
                var resp = asClient.SetInstanceProtectionAsync(req).GetAwaiter().GetResult();
                if (resp.HttpStatusCode != HttpStatusCode.OK)
                {
                    throw new Exception($"HTTP {resp.HttpStatusCode}");
                }
