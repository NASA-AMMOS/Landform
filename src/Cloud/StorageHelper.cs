...
namespace JPLOPS.Cloud
{
    ...
    public class StorageHelper : IDisposable
    {
                        ...
                        foreach (var regions in new [] { shortlist /*, RegionEndpoint.EnumerableAllRegions*/ })
                        {
                            foreach (var region in regions)
                            {
                                try
                                {
                                    ...
                                    GetBucketLocationResponse response = client.GetBucketLocationAsync(request).GetAwaiter().GetResult();
                    ...
                    foreach (S3Object entry in response.S3Objects)
                    {
                        ...
                        if (regex.IsMatch(key))
                        {
                            ...
                            if (filter == null || filter(url))
                            {
                                if (metadata != null)
                                {
                                    metadata(url, entry.Size ?? 0, entry.LastModified ?? DateTime.MaxValue);
                                }
                                yield return url;
                            }
                        }
        ...
        {
            return GetObjectMetadata(client, location).LastModified ?? DateTime.MaxValue;
        }
        ...
        public bool FileExists(string s3url)
        {
            ...
            try
            {
                GetClient(s3url).GetObjectMetadataAsync(new GetObjectMetadataRequest()
                {
        ...
        public bool DeleteObject(string s3url, bool ignoreErrors = true, ILog logger = null)
        {
            ...
            {
                S3Url location = new S3Url(s3url);
                GetClient(s3url).DeleteObjectAsync(location.BucketName, location.Path).GetAwaiter().GetResult();
                return true;
            }
            catch (Exception e)
            {
                                  ...
                                  bool ignoreErrors = true, ILog logger = null)
        {
            ...
            try
            {
                ...
                GetClient(s3url).DeleteObjectsAsync(request).GetAwaiter().GetResult();
            }
            catch (Exception e)
            {
                if (!ignoreErrors)
        ...
        private GetObjectMetadataResponse GetObjectMetadata(AmazonS3Client client, S3Url location)
        {
            ...
            var meta = client.GetObjectMetadataAsync(getObjectMetadataRequest).GetAwaiter().GetResult();
            return meta;
        }
        ...
        public class StorageStream : Stream
        {
            ...
            private long GetSize()
            {
                ...
                ListObjectsV2Response response = client.ListObjectsV2Async(request).GetAwaiter().GetResult();
                ...
                return response.S3Objects[0].Size ?? 0;
            }
