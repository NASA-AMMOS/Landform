...
namespace PipelineTest
{
    public class SplitCriteriaTests
    {
        ...
        public void FaceShouldSplitTest()
        {
            ...
            Assert.True(!string.IsNullOrEmpty(split.ShouldSplit(box, op)));

            BoundingBox quarterBox = new BoundingBox(Vector3.Zero, Vector3.One);
            Assert.False(!string.IsNullOrEmpty(split.ShouldSplit(quarterBox, op)));
        }
        ...
        public void TextureSplitTest()
        {
            ...
            Assert.True(StandardTexSplit(destTextureResolution, srcImageResolution, approx: false), msg);
            Assert.True(StandardTexSplit(destTextureResolution, srcImageResolution, approx: true), msg + " approx");
            ...
            Assert.False(StandardTexSplit(destTextureResolution, srcImageResolution, false), msg);
            Assert.True(StandardTexSplit(destTextureResolution, srcImageResolution, true), msg + " approx");

            srcImageResolution = 250; //62500 pixels / m^2
            msg = $"src res {srcImageResolution}, dst res {destTextureResolution}";
            Assert.False(StandardTexSplit(destTextureResolution, srcImageResolution, false), msg);
            Assert.False(StandardTexSplit(destTextureResolution, srcImageResolution, true), msg + " approx");
        }
