...
namespace ImagingTest
{
    public class GDALSerializerTest
    {
        ...
        {
            Image pattern = new GDALSerializer().Read("testPattern.png", ImageConverters.PassThrough);
        ...
        {
            Image pattern = new GDALSerializer().Read("testPattern.png", ImageConverters.ValueRangeToNormalizedImage);
        ...
        public void GDALTestFillValue()
        {
            ...
            for (int r = 0; r < pattern.Height; r++)
            {
                for (int c = 0; c < pattern.Width; c++)
                {
                    ...
                    Assert.Equal(shouldBeMasked, !maskedImg.IsValid(r, c));
            ...
            for (int r = 0; r < pattern.Height; r++)
            {
                for (int c = 0; c < pattern.Width; c++)
                {
                    ...
                    Assert.True(nonMaskedImg.IsValid(r, c));
                }
            }
        }
    }
