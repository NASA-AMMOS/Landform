...
//MP
        ...
        public static void MoveFileAtomic(string src, string dst)
        {
            ...
            EnsureExists(Path.GetDirectoryName(dstFull));

            bool sameFS = string.Equals(Path.GetPathRoot(srcFull), Path.GetPathRoot(dstFull),
                                        StringComparison.OrdinalIgnoreCase);

            string tmp = dstFull + ".tmp";
            ...
            try
            {
                ...
                {
                    File.Copy(srcFull, tmp, true); //tmp is a copy of srcFull that is on the same filesystem as dst
                    File.Move(tmp, dstFull, overwrite: true);
                    File.Delete(srcFull);
                }
            }
            catch (Exception ex)
            {
                throw new IOException(string.Format("error moving {0} (exists={1}) to {2} (exists={3}): {4}",
                                                    srcFull, File.Exists(srcFull), dstFull, File.Exists(dstFull),
                                                    ex.Message));
            }
            finally
            {
            ...
            {
                throw new IOException(string.Format("error moving {0} to {1}: move failed", srcFull, dstFull));
            }
        }
