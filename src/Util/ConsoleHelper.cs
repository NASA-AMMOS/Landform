...
#else
//MP see discussion below for freeVirt
//            totVirt = System.GC.GetGCMemoryInfo().TotalAvailableMemoryBytes / (1024L * 1024L);
#endif
            return totVirt;
        }
...
#else
//MP
//on other platforms we could in theory do things like shell out to a command like Linux "free"
//if we want to stay within dotnet the best we might be able to portably do is something like the following
//but this is not really answering the original question; instead it's telling us how much more heap this
//particular process could use
//for now just return -1 which will disable the memory watchdog in LandformService.cs
//            freeVirt = (System.GC.GetGCMemoryInfo().TotalAvailableMemoryBytes - System.GC.GetTotalMemory(false)) /
//                (1024L * 1024L);
#endif
            return freeVirt;
        }
