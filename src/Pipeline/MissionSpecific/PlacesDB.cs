    ...
    public class PlacesDB
    {
        ...
        private string Fetch(string query, bool throwOnError = true)
        {
            lock (cache)
            {
                ...
                {
                    options.Authenticator = new HttpBasicAuthenticator(config.Username, config.APIKey);
                }
                ...
                if (!string.IsNullOrEmpty(config.AuthCookieName) && !string.IsNullOrEmpty(cookieValue))
                {
                    ...
                    options.CookieContainer.Add(cookie);
                }
                ...
                if (maxSec > 0)
                {
                    request.Timeout = TimeSpan.FromSeconds(maxSec);
                }
