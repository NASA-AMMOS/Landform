    ...
    abstract public class TextureSplitCriteria : ITileSplitCriteria
    {
        ...
        public bool ShouldSplit(MeshOperator meshOperator, BoundingBox areaOfInterest)
        {
            ...
            List<CameraInstance> intersectingCameras =
                options.cameraInstances.Where(ci => ci.hullInMesh.Intersects(areaOfInterest)).ToList();
            ...
            if (!GetTileTexelsPerArea(clippedMesh, res, meshArea, out double texels))
            {
                return false;
            }

            if (!GetObservationPixelsPerArea(clippedMesh, res, clippedHull, intersectingCameras, out double pixels))
            {
                return false;
            }

            bool shouldSplit = pixels / texels > options.maxPixelsPerTexel;

            if (spewProgress && options.progress != null)
            {
    ...
    public class TextureSplitCriteriaBackproject : TextureSplitCriteria
    {
                                                            ...
                                                            out double pixels)
        {
            ...
            if (!clippedMesh.HasUVs)
            {
                try
                {
                    clippedMesh = UVAtlas.Atlas(clippedMesh, texRes, texRes, maxStretch: options.maxTextureStretch,
        ...
        private bool GetBestCameraByPixelDensity(List<CameraInstance> candidateCameras, ConvexHull meshHull,
                                                 BoundingBox meshBounds, PixelPoint pxlPt,
                                                 out CameraInstance bestCamera)
        {
            bestCamera = null;
            ...
            foreach (var camInst in candidateCameras)
            {
                ...
                double curSpread = ProjectedPixelDistances.
                    GetMinPixelSpreadInMeters(options.scInMesh, camInst.cameraModel, camInst.cameraToMesh,
    ...
    public class TextureSplitCriteriaApproximate : TextureSplitCriteria
    {
                                                            ...
                                                            out double pixels)
        {
            ...
            foreach (var camInst in intersectingCameras)
            {
                ...
                double curAreaPerPixelInMeters =
                    Vector3.Distance(ptUpperLeftCorner, ptUpperRightCorner) *
                    Vector3.Distance(ptUpperLeftCorner, ptLowerLeftCorner);
