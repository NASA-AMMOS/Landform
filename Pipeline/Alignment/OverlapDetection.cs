    ...
    class OverlapDetection
    {
        ...
        public bool ProjectiveFrustumOverlap(RoverObservation obs0, RoverObservation obs1)
        {
            ...
            Vector3[] cornerPoints = new Vector3[] {
                Vector3.Transform(model1.Backproject2DTo3D(new Vector2(0, 0), 0.01), oneToZero),
                Vector3.Transform(model1.Backproject2DTo3D(new Vector2(width1, 0), 0.01), oneToZero),
                Vector3.Transform(model1.Backproject2DTo3D(new Vector2(width1, height1), 0.01), oneToZero),
                Vector3.Transform(model1.Backproject2DTo3D(new Vector2(0, height1), 0.01), oneToZero),
                Vector3.Transform(model1.Backproject2DTo3D(new Vector2(0, 0), 40.0), oneToZero),
                Vector3.Transform(model1.Backproject2DTo3D(new Vector2(width1, 0), 40.0), oneToZero),
                Vector3.Transform(model1.Backproject2DTo3D(new Vector2(width1, height1), 40.0), oneToZero),
                Vector3.Transform(model1.Backproject2DTo3D(new Vector2(0, height1), 40.0), oneToZero)
            };
            ...
            {
                double z;
                Vector2 projected = model0.Project3DTo2D(pt, out z);
                if (z > maxZ) maxZ = z;
