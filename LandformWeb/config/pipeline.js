...
module.exports = () => {
  ...
  pipelineRouter.get('/env', (req, res) => {
    const vars = {
  ...
  function queueLength(queueUrl, res) {
    ...
    sqs.getQueueAttributes(params, (err, data) => {
      if (err) {
        ...
        res.status(200).send({
          QueueSize: data.Attributes.ApproximateNumberOfMessages,
        });
      }
    });
  }

  pipelineRouter.get('/align', (req, res) => {
    queueLength(process.env.ALIGN_JOB_QUEUE, res);
  });

  pipelineRouter.get('/tiling', (req, res) => {
    queueLength(process.env.TILING_JOB_QUEUE, res);
  });
  ...
  pipelineRouter.get('/align/workers', (req, res) => {
    workerCount(process.env.ALIGN_WORKERS, res);
  });
  pipelineRouter.get('/tiling/workers', (req, res) => {
    workerCount(process.env.ALIGN_WORKERS, res);
  });
