...
1. AWS credentials will be required for any of the next steps.
...
1. Optional - test the Docker container locally.
    ...
    1. This will also require AWS credentials on your workstation, which will be mounted from the Docker container.  The credentials are required because the web server runs the tiling master as a subprocess.
    1. Check `venueName` in `config.js` - the server will still connect to live AWS services for that venue, even though you're running it locally.
...
1. Deploy to Elastic Beanstalk
    1. Check `venueName` in `config.js` - the server will connect to AWS services for that venue.
...
1. Note: in order to run projects you will also need at least one tiling worker connected to the same AWS venue.
    1. To run the tiling worker locally:
